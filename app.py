...
def before_request():
    ...
    if not hasattr(session, 'initialized'):
        # Open the file in append mode when the app starts
        with open(file_path, "a") as output_file:
            output_file.write("")

        # Set the initialized attribute to avoid running this again
...
def get_bot_response():
    ...
    if session['current_question_index'] < len(questions):
        ...
        return jsonify({'message': "All questions have been asked. Sentiment scores are stored in sentiment_scores.txt."}), 400
